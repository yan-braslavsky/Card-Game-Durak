...
public class RetaliationInvalidMsgSubProcessor extends BaseMsgSubProcessor<RetaliationInvalidProtocolMessage> {
    ...
    @Override
    public void processMessage(final RetaliationInvalidProtocolMessage serverMessage) {
        ...
        for (final RetaliationSetData retaliationSetData : serverMessage.getMessageData().getInvalidRetaliationsList()) {

            final PileModel fieldPile = pileManagerService.findFieldPileWithCardByRankAndSuit(retaliationSetData.getCoveringCardData().getRank(), retaliationSetData.getCoveringCardData().getSuit());
            final Card coveringCard = fieldPile.findCardByRankAndSuit(retaliationSetData.getCoveringCardData().getRank(), retaliationSetData.getCoveringCardData().getSuit());
